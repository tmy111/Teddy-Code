...
from teddycode import Engine, TeddyCode, SessionEventBus, SessionStore, WorkspaceContext
...
def test_engine_drives_real_session_and_persists_event_timeline(tmp_path):
    ...
    assert (
        ...
        == tmp_path / ".teddycode" / "sessions" / f"{agent.session['id']}.events.jsonl"
    )
...
def test_plan_mode_allows_only_the_active_plan_artifact_until_plan_is_written(tmp_path):
    ...
    assert plan_path == ".teddycode/plans/v3-plan.md"
    ...
    assert (
        (tmp_path / ".teddycode" / "plans" / "v3-plan.md")
...
def test_plan_mode_tools_enter_and_exit_runtime_mode(tmp_path):
    ...
    assert ".teddycode/plans/refactor-auth-plan.md" in entered
    assert agent.runtime_mode == "plan"
...
def test_plan_path_accepts_absolute_path_inside_workspace(tmp_path):
    """模型偶尔给绝对路径，如 /Users/u/repo/.teddycode/plans/foo —— 自动相对化，
    不应该让 agent 多走一次重试。"""
    from teddycode.core.plan_mode import _plan_path

    assert (
        _plan_path("Student Mgmt", "/Users/u/repo/.teddycode/plans/student-mgmt.md")
        == ".teddycode/plans/student-mgmt.md"
    )
    assert (
        _plan_path("X", "./.teddycode/plans/x-plan.md") == ".teddycode/plans/x-plan.md"
    )
    # 真正越界的还是要拒
    import pytest
