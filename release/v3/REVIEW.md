# TeddyCode v3 review pack

## Project pitch

TeddyCode is a small local coding agent that turns one user request into a bounded
...
- `TeddyCode` owns session state, memory, tools, and workspace safety.
...
be validated through persisted `.teddycode/sessions/*.json` and
`.teddycode/sessions/*.events.jsonl` artifacts before treating a runtime change as
done.
...
- `.teddycode/runs/<run_id>/task_state.json`
- `.teddycode/runs/<run_id>/trace.jsonl`
- `.teddycode/runs/<run_id>/report.json`
